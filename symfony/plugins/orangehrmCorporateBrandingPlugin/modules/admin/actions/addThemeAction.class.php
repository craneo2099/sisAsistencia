...
class addThemeAction extends sfAction
{
    ...
    public function execute($request)
    {
        ...
        if ($request->isMethod("POST")) {
            ...
            $mainLogoWidth = empty($files['file']['tmp_name'])?0:getimagesize($files['file']['tmp_name'])[0];
            $mainLogoHeight = empty ($files['file']['tmp_name'])?0:getimagesize($files['file']['tmp_name'])[1];
            $loginBannerWidth = empty ($files['loginBanner']['tmp_name'])?0:getimagesize($files['loginBanner']['tmp_name'])[0];
            $loginBannerHeight = empty ($files['loginBanner']['tmp_name'])?0:getimagesize($files['loginBanner']['tmp_name'])[1];
            ...
            $mainLogo = empty($files['file']['tmp_name'])?null:file_get_contents($files['file']['tmp_name']);
            $loginBanner = empty($files['loginBanner']['tmp_name'])?null:file_get_contents($files['loginBanner']['tmp_name']);
